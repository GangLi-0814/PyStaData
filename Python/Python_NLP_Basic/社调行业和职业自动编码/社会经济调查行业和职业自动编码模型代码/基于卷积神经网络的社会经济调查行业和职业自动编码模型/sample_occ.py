...
# 原数据和增加的样本路径
filename1 = u'data/occ/occ_raw.txt'
filename2 = u'data/occ/occ_da.txt'
filename3 = u'data/occ/occ_da0.txt'
...
# 将数据集划分为训练集,验证集和测试集
row = df1.shape[0]
df1.iloc[:int(row*0.85)].to_csv('data/occ/occ_train.txt',mode='ab+',index=False,sep='\t',encoding="utf_8_sig",header=0)
df1.iloc[int(row*0.85):int(row*0.9)].to_csv('data/occ/occ_val.txt',mode='ab+',index=False,sep='\t',encoding="utf_8_sig",header=0)
df1.iloc[int(row*0.9):].to_csv('data/occ/occ_test.txt',mode='ab+',index=False,sep='\t',encoding="utf_8_sig",header=0)
