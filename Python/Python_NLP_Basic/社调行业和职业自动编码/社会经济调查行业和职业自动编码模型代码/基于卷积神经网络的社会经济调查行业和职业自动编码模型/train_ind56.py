...
# 行业训练集,验证集
train_dir = os.path.join(ind_base_dir, 'ind_train_56.txt')
val_dir = os.path.join(ind_base_dir, 'ind_val_56.txt')
ind_train_dir = 'data/ind/ind_train.txt'

# 最佳验证结果保存路径
save_dir = 'checkpoints/textcnn/ind'
save_path = os.path.join(save_dir, 'best_validation_ind_56')
...
def train():
    ...
    # 配置 Tensorboard
    tensorboard_dir = 'tensorboard/textcnn/ind_56'
    ...
    x_train, y_train = process_file(train_dir, word_to_id, ind_df2, config.seq_length)
    x_val, y_val = process_file(val_dir, word_to_id, ind_df2, config.seq_length)
...
if __name__ == '__main__':
    ...
    config = TCNNConfig(ind_df2.shape[0], vocab_size)#mid
