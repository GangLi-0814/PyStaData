...
threshold1 = 5  # 不同数据增强方法选择的判断阈值



# 读取数据集
occfile = r'data/occ/occ_raw.txt'
df = pd.DataFrame(pd.read_table(occfile,sep='\t',encoding="utf_8_sig",names=['1','2']))

# 读取分类说明文件
classfile = u'data/classes.xls'
classdf = pd.DataFrame(pd.read_excel(classfile,sheetname=u'职业',encoding='utf_8_sig'))
...
newdata0.to_csv('data/occ/occ_da0.txt',mode='ab+',index=False,sep='\t',encoding="utf_8_sig",header=0)
...
newdata.to_csv('data/occ/occ_da.txt',mode='ab+',index=False,sep='\t',encoding="utf_8_sig",header=0)
