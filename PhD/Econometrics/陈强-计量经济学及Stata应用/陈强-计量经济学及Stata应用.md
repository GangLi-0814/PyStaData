...
时间序列分为平稳时间序列和非平稳时间序列，需要使用不同的计量方法。
...
在 AR(p) 模型中引入其他解释变量，构成 ADL(p, q) 或 ARDL(p,q) 

p 为 y 的自回归阶数，q 为 x 的滞后阶数

滞后阶数的选择可以使用信息准则（AIC 或者 BIC），或者进行序贯检验，即使用 t 或 F 检验来检验最后一阶系数的显著性。

### 13.5 误差修正模型

从经济理论而言，相关的变量之间可能存在长期的均衡关系，而变量的短期变动则向着长期均衡关系的部分调整。

误差修正模型的经济含义明确，可分别考察长期效应（长期均衡关系）和短期效应（误差修正效应）。
