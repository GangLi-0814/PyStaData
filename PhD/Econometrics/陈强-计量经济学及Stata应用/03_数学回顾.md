...
转置矩阵（transpose matrix），记为$A'$（A prime）

行向量（row vector）

列向量（column vector）

如果$a'b=0$，则称向量 $a$ 和向量 $b$ 正交（orthogonal），意味着在$n$维向量空间中相互垂直。

**逆矩阵（inverse matrix）：对于 $n$ 级方阵 $A$ ，如果存在 $n$ 级方阵 $B$ ，使得 $AB=BA=I_n$，则称 $A$ 为可逆矩阵（invertible matrix），或非退化矩阵（nonsingular matrix），而 $B$ 为 $A$ 的逆矩阵（inverse matrix），记为 $A^{-1}$ 。**
...
在做矩阵乘法时，需区分**左乘**（premultiplication）与**右乘**（postmultiplication）。$A$ 左乘 $B$为 $AB$，而 $A$ 右乘 $B$ 为 $BA$ 。
...
如果$\{a_1,a_2,\dots,a_K\}$ 线性相关，则其中至少有一个向量可写为其他向量的线性组合（linear combination），也称线性表出。

向量组 $\{a_1,a_2,...,a_K\}$ 的极大线性无关部分组所包含的向量个数称为该**向量的秩（Rank）**。

#### 3.2.11 二次型

对于n维列向量，如何度量它与零向量的距离？最简单的方式是欧几里德距离（Euclidean distance），即内积。
...
此二次型可以写为：
$$
f(x_1,x_2)=(x_1 \quad x_2)\begin{Bmatrix}a_{11} & a_{12} \\a_{21} & a_{22} \end{Bmatrix} \begin{Bmatrix} x_1 \\ x_2\end{Bmatrix} = x'Ax
$$
其中，$a_{21} = a_{12}$ 。
...
在 $B$ 发生的条件下，$A$ 发生的条件概率（conditional probability）为：
...
假设随机变量 $X$ 的可能取值为 $\{x_1,x_2,\dots,x_k,\dots\}$，其对应概率为$\{p_1,p_2,\dots,p_k,\dots\}$, $p_k \equiv P(X=x_k)$，则称$X$ 为离散型随机变量。

常见的离散部分有两点分布（Bernoulli）、二项分布（Binomial）、泊松分布（Poisson）等。

#### 3.4.2 连续型概率分布

连续型随机变量$X$ 可以取任意实数，其概率密度函数$f(x)$ 满足：

1. $f(x) \geq 0, \forall x$;
2. $\int_{-\infin}^{+\infin} f(x)dx =1$;
3. $X$ 落在区间$[a,b]$ 的概率为 $P(a\leq X\leq b)=\int_b^a f(x)dx$ 

<img src="/Users/gangli/PyStaData/PhD/Econometrics/陈强-计量经济学及Stata应用/images/3-9.png" />

累积分布函数（cdf）：
$$
F(x)\equiv P(-\infin < X \leq x) = \int_{-\infin}^x f(t)dt
$$
​	其中，$t$ 为积分变量。$F(x)$ 度量的是从$-\infin$ 到 $x$ 为止，概率密度函数$f(t)$ 曲线下面的面积。

<img src="/Users/gangli/PyStaData/PhD/Econometrics/陈强-计量经济学及Stata应用/images/3-10.png" />

#### 3.4.3 多维随机向量的概率分布

为研究变量间关系，常**同时**考虑两个或多个随机变量，即随机向量（random vector）；二维连续型随机向量（$X$, $Y$）联合密度函数（joint pdf）满足：

1. $f(x,y) \geq 0, \forall x,y$;
2. $\int_{-\infin}^{+\infin} f(x,y)dxdy=1$;
3. $(X,Y)$ 落入平面某区域$D$的概率为：$P\{(X,Y) \in D \}=\int\int_D f(x,y)dxdy$

$X$的边缘密度函数（marginal pdf）：
$$
f_x(x) = \int_{-\infin}^{+\infin}f(x,y)dy
$$
即给定$X=x$ ，把所有$Y$取值的可能性都“加总”起来。

#### 3.4.4 条件分布

#### 3.4.5 随机变量的数字特征

连续型随机变量的**期望**：$E(X)\equiv\mu\equiv\int_{-\infin}^{+\infin}xf(x)dx$，即对$x$进行加权平均，权重为概率密度$f(x)$。

随机变量$X$的方差：$Var(X)\equiv \sigma^2\equiv E[X-E(x)]^2$

协方差：$Cov(X,Y)\equiv \sigma_{XY}\equiv E[(X-E(X))(Y-E(Y))]$

相关系数：$\rho\equiv Corr(X,Y)\equiv\frac{Cov(X,Y)}{\sqrt{Var(X)Var(Y)}}=\frac{\sigma_{XY}}{\sigma_X\sigma_Y}$
