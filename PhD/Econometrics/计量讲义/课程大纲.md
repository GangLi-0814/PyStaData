...
方差分析(analysis of variance)，简写为ANOVA，指的是**利用对多个样本的方差的分析，得出总体均值是否相等的判定。**当回归模型中包含的解释变量都是虚拟变量时，这种模型被称为方差分析模型。

自由度

拟合优度

证明 ESS + RSS = TSS 。（关键在 $Cov(\hat u_i,y_i) = 0$ ）
